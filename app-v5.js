...
function bindImagePreviewModal(scope = document){
  scope.querySelectorAll("[data-preview-src]").forEach((el) => {
    if(el.dataset.previewBound === "1") return;
    el.dataset.previewBound = "1";
    el.onclick = () => openImageLightbox(el.dataset.previewSrc || el.getAttribute("src") || "");
  });
}
...
function initImageLightbox(){
  const closeBtn = $("closeImageLightboxBtn");
  const backdrop = $("imageLightboxBackdrop");
  const modal = $("imageLightbox");
  if(closeBtn) closeBtn.onclick = closeImageLightbox;
  if(backdrop) backdrop.onclick = closeImageLightbox;
  if(modal) modal.onclick = (e) => { if(e.target.id === "imageLightbox") closeImageLightbox(); };
}
...
async function saveInquiryMessage(payload){
  ...
  const starter = { sender:"customer", text: payload.message, image: payload.image || "", at: now, deliveredAt: now };
  ...
  if(getMode()==="firebase" && firebaseReady){
    await addDoc(collection(db, "messages"), {
      ...
      latestImage: payload.image || "",
      latestMessage: payload.message || (payload.image ? "📷 Image" : ""),
      status:"New",
      reply:"",
      latestImage: payload.image || "",
      thread:[starter],
...
function updateInboxBadge(messages, account = {}){
  ...
  messages.forEach(msg => {
    const belongs = (chatId && msg.id === chatId) || (phone && String(msg.phone || "") === phone);
    if(!belongs) return;
    const thread = Array.isArray(msg.thread) ? msg.thread : [];
    count += thread.filter(item => item.sender === "admin" && !item.customerSeenAt).length;
  });
...
function initShop(){
  ...
  function renderCustomerChat(conversation){
    ...
    box.innerHTML = thread.map((item, index) => `
      <div class="chat-bubble ${item.sender === "admin" ? "admin" : "customer"}">
        ${item.image ? `<img class="chat-image" src="${item.image}" data-preview-src="${item.image}" alt="Chat image" />` : ""}
        ${item.text ? `<div>${nl2br(item.text)}</div>` : ""}
        <span class="chat-meta">${item.sender === "admin" ? "Admin" : "You"} • ${escapeHtml(String(item.at || "").replace("T"," ").slice(0,16))}${item.sender === "customer" && index === thread.length - 1 ? ` • ${escapeHtml(item.seenAt ? "Seen" : (item.deliveredAt ? "Delivered" : "Sent"))}` : ""}</span>
      </div>
    `).join("");
    bindImagePreviewModal(box);
  ...
  async function loadCustomerConversation(){
    ...
    if(conversation) {
      localStorage.setItem(CHAT_ID_KEY, conversation.id);
      const thread = Array.isArray(conversation.thread) ? conversation.thread : [];
      const latestCustomer = [...thread].reverse().find(item => item.sender === "customer");
      if(latestCustomer && !latestCustomer.seenAt){
        latestCustomer.deliveredAt = latestCustomer.deliveredAt || new Date().toISOString();
      }
    }
  ...
  async function sendInquiry(){
    ...
    const image = customerPendingImage || "";

    if(!name || !phone || (!message && !image)){
    ...
    try{
      ...
      if(current){
        ...
        thread.push({ sender:"customer", text: message, image, at: now, deliveredAt: now });
        await updateMessage(current.id, {
          thread,
          latestMessage: message || (image ? "📷 Image" : ""),
          latestImage: image,
          message,
          status:"New",
          updatedAt: now
      ...
      if($("inqImagePreviewWrap")) $("inqImagePreviewWrap").classList.add("hidden");
      if($("inqImagePreview")) $("inqImagePreview").src = "";
      if($("inq_image")) $("inq_image").value = "";
  ...
  bindNoticeButtons();
  initImageLightbox();
  subscribeMessages((messages) => updateInboxBadge(messages, account));

  if($("inq_image")) $("inq_image").onchange = async (e) => {
    const file = e.target.files && e.target.files[0];
    if(!file) return;
    try{
      customerPendingImage = await compressImageFile(file);
      $("inqImagePreview").src = customerPendingImage;
      $("inqImagePreviewWrap").classList.remove("hidden");
    }catch{
      showNotice("Image failed to load");
    }
  };
  if($("removeInqImageBtn")) $("removeInqImageBtn").onclick = () => {
    customerPendingImage = null;
    if($("inqImagePreviewWrap")) $("inqImagePreviewWrap").classList.add("hidden");
    if($("inqImagePreview")) $("inqImagePreview").src = "";
    if($("inq_image")) $("inq_image").value = "";
  ...
  if($("sendProductToChatBtn")) $("sendProductToChatBtn").onclick = () => {
    if(!selectedProduct){ showNotice("No product selected"); return; }
    const qty = detailQty || 1;
    const size = selectedSize || "Not selected yet";
    const prefill = `Hi, I want to ask about this product:
...
Please help me with custom design details.`;
    closeProductPage();
    openInquiry(prefill);
  };
...
function initAdmin(){
  bindNoticeButtons(); initImageLightbox(); const form = $("productForm"), table = $("adminProductTable"); let activeOrdersCache = [];
  ...
  function renderMessages(messages){
    ...
    const preparedThread = thread.map(item => {
      if(item.sender === "admin" && !item.customerSeenAt){
        seenChanged = true;
        return { ...item, customerSeenAt: new Date().toISOString() };
      }
      if(item.sender === "customer" && !item.seenAt){
        return { ...item, seenAt: new Date().toISOString() };
      }
      return item;
    });
    if(seenChanged){
      updateMessage(current.id, { thread: preparedThread, updatedAt: new Date().toISOString() }).catch(() => {});
    }
    chat.innerHTML = preparedThread.length ? preparedThread.map(item => `
      <div class="chat-bubble ${item.sender === "admin" ? "admin" : "customer"}">
        ${item.image ? `<img class="chat-image" src="${item.image}" data-preview-src="${item.image}" alt="Chat image" />` : ""}
        ${item.text ? `<div>${nl2br(item.text)}</div>` : ""}
    ...
    sendBtn.onclick = async () => {
      ...
      if(!text && !image){
        showNotice("Type a reply or add an image first");
        return;
      }
      try{
        const now = new Date().toISOString();
        const newThread = preparedThread.concat([{ sender:"admin", text, image, at: now }]);
        await updateMessage(current.id, {
          thread: newThread,
          reply: text,
        ...
        if($("adminReplyImagePreviewWrap")) $("adminReplyImagePreviewWrap").classList.add("hidden");
        if($("adminReplyImagePreview")) $("adminReplyImagePreview").src = "";
        if($("adminReplyImage")) $("adminReplyImage").value = "";
  ...
  if($("adminReplyImage")) $("adminReplyImage").onchange = async (e) => {
    const file = e.target.files && e.target.files[0];
    if(!file) return;
    try{
      adminPendingImage = await compressImageFile(file);
      $("adminReplyImagePreview").src = adminPendingImage;
      $("adminReplyImagePreviewWrap").classList.remove("hidden");
    }catch{
      showNotice("Image failed to load");
    }
  };
  if($("removeAdminReplyImageBtn")) $("removeAdminReplyImageBtn").onclick = () => {
    adminPendingImage = null;
    if($("adminReplyImagePreviewWrap")) $("adminReplyImagePreviewWrap").classList.add("hidden");
    if($("adminReplyImagePreview")) $("adminReplyImagePreview").src = "";
    if($("adminReplyImage")) $("adminReplyImage").value = "";
